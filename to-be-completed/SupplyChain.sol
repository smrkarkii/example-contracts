...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier sold(uint256 sku){ 
    require (items[sku].state == State.Sold,"Item not sold yet");
     _;
     }
  ...
  modifier received(uint256 sku){ 
    require (items[sku].state == State.Received, "Item not recieved ");
     _;
     }
    ...
    public payable forSale()
  {
    ...
    require(msg.value >= items[sku].price,"Insufficient amount");
      ...
      address payable seller = items[sku].seller;
        ...
        uint excess = msg.value-items[sku].price;
        account.transfer(excess);
        emit LogSold(sku);
